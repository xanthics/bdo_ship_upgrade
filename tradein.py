# a dictionary of barter item trade-ins

barters = {
	"Enhanced Island Tree Coated Plywood": [{"count": 10, "input": ["[Level 4] Marine Knights' Spear", "[Level 4] Bronze Candlestick", "[Level 4] Stolen Pirate Dagger"]}],
	"Luminous Cobalt Ingot": [{"count": 1, "input": ["[Level 4] Stolen Pirate Dagger", "[Level 4] Solidified Lava", "[Level 4] Pirate's Key"]}],
	"Cox Pirates' Artifact (Parley Expert)": [{"count": 1, "input": ["[Level 4] Amethyst Fragment", "[Level 4] Headless Dragon Figurine"]}],
	"Bright Reef Piece": [{"count": 4, "input": ["[Level 3] Pirates' Supple Box", "[Level 3] Weasel Leather Coat", "[Level 3] Lopters Fishnet"]}],
	"Pure Pearl Crystal": [{"count": 1, "input": ["[Level 3] Torn Pirate Treasure Map", "[Level 3] Scout Binoculars", "[Level 3] Skull Symbol Carpet", "[Level 3] Ancient Orders"]}],
	"Great Ocean Dark Iron": [{"count": 5, "input": ["[Level 4] Opulent Thread Spool", "[Level 4] Green Salt Lump", "[Level 4] Panacea"]}],
	"Cox Pirates' Artifact (Parley Beginner)": [{"count": 1, "input": ["[Level 2] Opulent Marble", "[Level 2] Big Stone Slab", "[Level 2] Monster Tentacle"]}],
	"Cobalt Ingot": [{"count": 2, "input": ["[Level 2] Pirate Ship Mast", "[Level 2] Supreme Oyster Box", "[Level 2] Sea Survival Kit"]}],
	"Deep Sea Memory Filled Glue": [{"count": 4, "input": ["[Level 3] Torn Pirate Treasure Map", "[Level 3] Gooey Monster Blood", "[Level 3] Rare Herb Pile", "[Level 3] Blue Candle Bundle"]}],
	"Brilliant Rock Salt Ingot": [{"count": 1, "input": ["[Level 5] Portrait of the Ancient", "[Level 5] Luxury Patterned Fabric", "[Level 5] 102 Year Old Golden Herb", "[Level 5] Taxidermied White Caterpillar"]}],
	"Brilliant Pearl Shard": [{"count": 1, "input": ["[Level 5] Elixir of Youth", "[Level 5] Mysterious Rock", "[Level 5] Golden Fish Scale", "[Level 5] Faded Gold Dragon Figurine"]}],
	"Island Tree Coated Plywood": [{"count": 100, "input": ["[Level 2] Filtered Drinking Water", "[Level 2] Narvo Sea Cucumber", "[Level 2] Pirate Gold Coin"]}],
	"Rock Salt Ingot": [{"count": 100, "input": ["[Level 2] Conch Shell Ornament", "[Level 2] Balanced Stone Pagoda", "[Level 2] Urchin Spine"]}],
	"Fiery Black Stone": [{"count": 1, "input": ["[Level 2] Cron Castle Gold Coin"]},
						  {"count": 2, "input": ["[Level 3] Stalactie Fragment", "[Level 3] Skull Decoration Teacup"]},
						  {"count": 3, "input": ["[Level 4] Boatman's Manual", "[Level 4] Old Chest With Gold Coins"]}],
	"Frosted Black Stone": [{"count": 1, "input": ["[Level 2] Cron Castle Gold Coin"]},
							{"count": 2, "input": ["[Level 3] Round Knife"]},
							{"count": 3, "input": ["[Level 4] Seashell Deco"]}],
	"Verdant Black Stone": [{"count": 1, "input": ["any level 1 barter good.  Exchange coupon with Ravinia"]}],
}
